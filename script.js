/* ═══════════ GLASSORA — Complete Logic ═══════════ */

// ─── Real Product Data (Bengali Market Prices) ───
const products = [
...
function renderProducts() {
    ...
    if (filtered.length === 0) {
        grid.innerHTML = `<div class="cart-empty" style="grid-column:1/-1;padding:80px">
            <i class="fas fa-search" style="font-size:60px;color:var(--text-mut)"></i>
            <p style="font-size:20px;font-weight:700;margin-top:15px">কোনো প্রোডাক্ট পাওয়া যায়নি!</p>
            <span style="color:var(--text-mut)">অন্য কিছু খুঁজুন</span></div>`;
        return;
    }
        ...
        <div class="product-card glass" style="animation-delay:${i * 0.1}s">
            <div class="product-image">
                ...
                <div class="product-badges">
                    ${p.badge === 'sale' ? '<span class="product-badge badge-sale">সেল</span>' : ''}
                    ${p.badge === 'new' ? '<span class="product-badge badge-new">নতুন</span>' : ''}
                    ${p.badge === 'hot' ? '<span class="product-badge badge-hot">হট</span>' : ''}
                </div>
                <button class="product-wishlist" onclick="toggleWishlist(${p.id}, this)" aria-label="Wishlist">
                    <i class="far fa-heart"></i>
                </button>
            </div>
            <div class="product-info">
                <div class="product-category">${p.category === 'electronics' ? 'ইলেকট্রনিক্স' : p.category === 'fashion' ? 'ফ্যাশন' : 'বিউটি'}</div>
                <h3 class="product-name">${p.name}</h3>
                <div class="product-rating">
                    <div class="stars">${'<i class="fas fa-star"></i>'.repeat(Math.floor(p.rating))}${p.rating % 1 !== 0 ? '<i class="fas fa-star-half-alt"></i>' : ''}</div>
                ...
                <div class="product-actions">
                    <button class="btn-add-cart" onclick="addToCart(${p.id})"><i class="fas fa-shopping-cart"></i> কার্টে যোগ করুন</button>
                    <button class="btn-quick-view" onclick="openQuickView(${p.id})" aria-label="Quick View"><i class="fas fa-eye"></i></button>
                </div>
            </div>
        </div>
    `).join('');
...
function updateCartUI() {
    ...
    if (cart.length === 0) {
        container.innerHTML = `<div class="cart-empty">
            ...
            <span>প্রোডাক্ট যোগ করুন</span></div>`;
        return;
    }
...
function removeFromCart(id) {
    ...
    updateCartUI();
    showNotification('প্রোডাক্ট কার্ট থেকে সরানো হয়েছে!');
}

function checkout() {
    if (cart.length === 0) {
        showNotification('কার্ট খালি! আগে প্রোডাক্ট যোগ করুন।');
        return;
    }
    showNotification('অর্ডার সফলভাবে সম্পন্ন হয়েছে! 🎉');
    cart = [];
    updateCartUI();
    closeCart();
}
...
function openQuickView(id) {
    ...
    document.getElementById('quickViewContent').innerHTML = `
        ...
        <div class="modal-details">
            <div class="product-category">${p.category === 'electronics' ? 'ইলেকট্রনিক্স' : p.category === 'fashion' ? 'ফ্যাশন' : 'বিউটি'}</div>
            <h2>${p.name}</h2>
            <div class="modal-price">৳${p.price.toLocaleString('en-IN')} <span style="font-size:16px;color:var(--text-mut);text-decoration:line-through">৳${p.originalPrice.toLocaleString('en-IN')}</span></div>
            <div class="product-rating">
                <div class="stars">${'<i class="fas fa-star"></i>'.repeat(Math.floor(p.rating))}</div>
                <span class="rating-count">(${p.reviews} রিভিউ)</span>
            </div>
            <p class="modal-desc">এটি একটি প্রিমিয়াম মানের অরিজিনাল প্রোডাক্ট। GLASSORA থেকে কেনার পর ৭ দিনের রিটার্ন পলিসি এবং ১ বছরের ওয়ারেন্টি গ্যারান্টি। সারাদেশে ফ্রি ও দ্রুত ডেলিভারি।</p>
            <div style="display:flex;gap:10px;margin-bottom:15px">
                <div style="flex:1;padding:10px;border-radius:10px;background:rgba(102,126,234,.1);text-align:center;font-size:13px;color:var(--accent1);font-weight:600"><i class="fas fa-check-circle"></i> অরিজিনাল</div>
                <div style="flex:1;padding:10px;border-radius:10px;background:rgba(16,185,129,.1);text-align:center;font-size:13px;color:var(--success);font-weight:600"><i class="fas fa-truck"></i> ফ্রি ডেলিভারি</div>
            </div>
            <button class="modal-add" onclick="addToCart(${p.id}); closeQuickView();"><i class="fas fa-shopping-cart"></i> কার্টে যোগ করুন</button>
        </div>`;
...
function toggleWishlist(id, btn) {
    ...
    const p = products.find(x => x.id === id);
    showNotification(btn.classList.contains('active') ? `${p.name} উইশলিস্টে যোগ হয়েছে! ❤️` : 'উইশলিস্ট থেকে অপসারিত হয়েছে');
}

function toggleWishlistBtn() {
    showNotification('উইশলিস্ট ফিচার চালু আছে!');
}
...
function handleSubscribe(e) {
    e.preventDefault();
    showNotification('নিউজলেটার সাবস্ক্রাইব সফল! 🎉');
    e.target.reset();
}

// ─── Countdown: 06:26:22 (23,182 seconds) ───
let totalSeconds = 23182;
function startCountdown() {
    ...
    function update() {
        if (totalSeconds <= 0) totalSeconds = 23182;
        totalSeconds--;
...
function toggleMobileMenu() {
    showNotification('মোবাইল মেনু — UI চালু আছে!');
}
